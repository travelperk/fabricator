...
type FabricatorType<T, X extends T = T> = {
  [P in keyof T]?: X[P] | (() => X[P])
}

export interface FabricatedValue<T extends object> {
  (modifier?: FabricatorType<T>): T
  extend: <U extends T>(
...
export function Fabricator<T extends object>(
  ...
  const fabricate = (opts?: FabricatorType<T>) => Fabricate(model, opts)
